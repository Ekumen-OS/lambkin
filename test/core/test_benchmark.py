...
def test_output_hook_called_once_after_all_iterations(variants):
    """Output hook is called once after the full benchmark loop."""
    call_count = 0
...
def test_output_hook_receives_last_ctx(variants):
    """Output hook receives the ctx of the last iteration."""
    seen_in_loop = []
    seen_in_output = []

    @benchmark(variants=variants, num_iterations=3)
    def nominal(ctx):
        seen_in_loop.append(ctx.iteration)

    @nominal.output
    def plots(ctx):
        seen_in_output.append(ctx.iteration)

    nominal(base_dir="/tmp")
    assert seen_in_loop[-1] == seen_in_output[0]


def test_output_hook_called_after_benchmark_loop(tmp_path):
    """Output hook runs after all benchmark iterations complete."""
    order = []

    @benchmark(variants=[{"sensor_model": "beam"}], num_iterations=2)
    def nominal(ctx):
    ...
    assert order == ["iteration", "iteration", "output"]
