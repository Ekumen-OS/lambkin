...
*** Settings ***
Documentation       Nominal Beluga vs Beluga benchmark using 2D datasets.

Resource            lambkin/shepherd/robot/resources/all.resource

Suite Setup         Setup Beluga vs Beluga benchmark suite
Suite Teardown      Teardown Beluga vs Beluga benchmark suite
Test Template       Run Beluga vs Beluga benchmark case for each ${dataset} ${basedir} ${laser_model} ${odom_frame} ${map_frame} ${base_frame} ${scan_topic} ${iterations} ${initial_pose_x} ${initial_pose_y} ${initial_pose_yaw} ${robot_model_type}
...
HQ Simulation             ${{HQ_SIMULATION_BAGS}}         ${{HQ_SIMULATION_DIR}}    ${{LASER_MODELS}}     odom           map        base_link       /scan          1              0.0             0.0             0.0                nav2_amcl::DifferentialMotionModel
...
Beluga vs Beluga benchmark suite
    ...
    Generates latexpdf report from nominal_report template in beluga_vs_beluga ROS 2 package

Beluga vs Beluga benchmark case
    ...
    Uses ${bagfile_path} at 2x as input to ROS 2 system
    ${package_share_path} =  Find ROS 2 Package  beluga_vs_beluga  share=yes
    ...
    # Setup benchmark rig
    Uses beluga_vs_beluga.launch in beluga_vs_beluga ROS package as rig
    ...
    Uses timemory-timem to sample beluga_amcl_alt performance
    # Setup benchmark analysis
    Tracks /beluga_amcl_alt/pose /beluga_amcl/pose trajectories
